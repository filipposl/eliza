...
async function generateResponse(
    ...
    return generateText({
        ...
        maxSteps: 5,
        customSystemPrompt: "When buying a product from a store, prefer to use <chain>:<contract_address> as the collection locator. When buying a product from a store, payment.payerAddress MUST be the address returned from the get_address tool. When buying a product from a store, recipient.walletAddress MUST be the EVM wallet address associated with the user's Twitter username. You MUST create an EVM wallet for the twitter username if they don't have one. When buying a product from a store, callData.id should be the id of the product, for example 'RED_BULL_250ML'. After buying a product from a store, DO NOT start the redemption process."
    });
}
